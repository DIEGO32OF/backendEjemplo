...
/**
 * V1.0 Diego Rivas
 * Helper / catalogo de respuestas de peticiones de la API estandarizadas
...
let languageElementsModel = require('./../models/languageElements.model')
...
let getErrorByCode = async function (code) {
    if (!code) {
        code = 500;
    }
    let response = RC['code_' + code];
    let list_of_elements = await languageElementsModel.find({reference: response.i18n})
        .populate({path: 'language', model: languageModel}).exec();

    var i18n = {};

    for (var i = 0; i < list_of_elements.length; i++) {
        let item = list_of_elements[i]

        if (item && item.language && item.language.iso_language_code) {
            i18n[item.language.iso_language_code] = item.text;
        }
    }
    response.global = i18n;
    return response
}

module.exports = {codes: RC, getErrorByCode};